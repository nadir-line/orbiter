...
void Atlantis::CreateAirfoils ()
{
	...
	CreateControlSurface (AIRCTRL_ELEVATOR, 8.0, 1.0, _V( 0, 0,  -15), AIRCTRL_AXIS_XPOS, anim_elev);
...
void Atlantis::VLiftCoeff (double aoa, double M, double Re, double *cl, double *cm, double *cd)
{
    ...
    static const double CLMachLow[nabsc] = {0, 0.25, 0.3, 0.25, 0.2, 0.1, 0, -0.2, -0.65, -1.1, -1.5, -0.7, 0, 0.7, 1.5, 1.1, 0.65, 0.2, 0, -0.1, -0.2, -0.25, -0.3, -0.25, 0};
        ...
        if (GetMachNumber() > 0.1 && GetAltitude(ALTMODE_GROUND) >= 100) {
            if (dap_entry_enabled) {
                ...
                if (pitch_mode == 1) { // pitch rate null mode
                    ...
                    elev_tgt = clamp(elev_tgt, -1.0, +1.0);
                    if (aoa_curr > 20 * RAD) {
                        elev_trim_tgt = 0.00045 * pow(aoa_tgt * DEG, 2);
